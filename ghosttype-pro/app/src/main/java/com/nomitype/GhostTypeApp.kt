...
class GhostTypeApp : Application() {
    private var clipboardWatcher: com.nomitype.utils.ClipboardWatcher? = null

    override fun onCreate() {
        super.onCreate()
        // Load native library if available (optional — app works without it)
        com.nomitype.security.NativeGuard.ensureLoaded()

        // Update gate — fetches remote app_version + download_url +
        //    app_enabled toggle.
        runCatching { com.nomitype.security.UpdateGate.check(this) }
        ...
        runCatching {
            val p = com.nomitype.utils.SettingsStore.prefs(this)
            val prev = p.getInt("build_version", 0)
            val cur = com.nomitype.BuildConfig.VERSION_CODE
            if (cur > prev) {
                com.nomitype.ime.AutoTypeEngine.stop()
                com.nomitype.ime.FloatingPointerService.stop(this)
                com.nomitype.ime.AutoTypeForegroundService.stop(this)
        ...
        runCatching {
            val prefs = com.nomitype.utils.SettingsStore.prefs(this)
            val enabled = prefs.getBoolean(com.nomitype.utils.SettingsStore.KEY_POINTER_ENABLED, false)
            if (enabled && android.provider.Settings.canDrawOverlays(this)) {
                com.nomitype.ime.FloatingPointerService.start(this)
            }
        }
        ...
        runCatching {
            ...
            if (!prefs.getBoolean("math_migrated_v1", false)) {
                val cur = prefs.getInt(com.nomitype.utils.SettingsStore.KEY_MATH_COUNT, 3)
                if (cur <= 1) {
                    prefs.edit()
                        .putInt(com.nomitype.utils.SettingsStore.KEY_MATH_COUNT, 3)
                        .apply()
                }
        ...
        runCatching {
            clipboardWatcher = com.nomitype.utils.ClipboardWatcher(this).also { it.start() }
        }
        ...
        runCatching {
            com.nomitype.security.ApprovalRefreshWorker.schedule(this)
            com.nomitype.security.ApprovalRefreshWorker.checkNow(this)
        }
        ...
        GlobalScope.launch(Dispatchers.IO) {
            runCatching { com.nomitype.security.ApprovalGate.evaluate(this@GhostTypeApp, force = true) }
        }
     ...
     * [com.nomitype.utils.SettingsStore.KEY_DEFAULTS_APPLIED]) so it never
    ...
    private fun applyDefaultsOnFirstRun() {
        ...
        com.nomitype.utils.ThemeManager.setTheme(this, "sky_cute_pastel")
        prefs.edit()
            .putString(com.nomitype.utils.SettingsStore.KEY_THEME, "sky_cute_pastel")
            .putString(com.nomitype.utils.SettingsStore.KEY_BG_IMAGE_URI, "")
            .putInt(com.nomitype.utils.SettingsStore.KEY_BG_IMAGE_OPACITY, 100)
            .putBoolean(com.nomitype.utils.SettingsStore.KEY_BG_SHOW_BORDERS, false)
            .putBoolean(com.nomitype.utils.SettingsStore.KEY_BG_IMAGE_ON_KEYS, false)
            .putString(com.nomitype.utils.SettingsStore.KEY_BORDER_STYLE, "rounded")
            .putInt(com.nomitype.utils.SettingsStore.KEY_KEY_OPACITY, 100)
            .putInt(com.nomitype.utils.SettingsStore.KEY_KEY_TEXT_SIZE, 18)
            .putInt(com.nomitype.utils.SettingsStore.KEY_KEY_HEIGHT_DP, 58)
            .putInt(com.nomitype.utils.SettingsStore.KEY_KEY_MARGIN_DP, 3)
            .putBoolean(com.nomitype.utils.SettingsStore.KEY_KEY_3D_SHADOW, true)
            .putInt(com.nomitype.utils.SettingsStore.KEY_MATH_COUNT, 3)
