...
import com.nomitype.security.DeviceId
import com.nomitype.utils.SettingsStore
import com.nomitype.utils.UnicodeFonts
...
class GhostTypeIMEService : InputMethodService() {
    ...
    private fun makeImeLockView(): View {
        ...
        tv.setOnClickListener {
            try {
                startActivity(
                    Intent(this, com.nomitype.ui.MainActivity::class.java)
    ...
    private val prefsListener = SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
        ...
        when (key) {
            SettingsStore.KEY_FONT_PATH, SettingsStore.KEY_FONT_BOLD, SettingsStore.KEY_FONT_ITALIC -> {
                ...
                com.nomitype.utils.FontManager.invalidateCache()
    ...
    private val approvalRevokedReceiver = object : android.content.BroadcastReceiver() {
        override fun onReceive(context: android.content.Context?, intent: android.content.Intent?) {
            if (intent?.action != com.nomitype.security.ApprovalRefreshWorker.ACTION_APPROVAL_REVOKED) return
            try {
    ...
    override fun onCreate() {
        ...
        try {
            val filter = android.content.IntentFilter(
                com.nomitype.security.ApprovalRefreshWorker.ACTION_APPROVAL_REVOKED
            )
    ...
    override fun onCreateInputView(): View {
        ...
        val approved = try { com.nomitype.security.ApprovalGate.isApprovedCached(this) }
            catch (_: Throwable) { false }
        val tamperOk = try { com.nomitype.security.SecurityGuard.verifyOrDie(this) }
            catch (_: Throwable) { false }
        val planExpired = try { com.nomitype.utils.SettingsStore.isPlanExpired(this) }
            catch (_: Throwable) { false }
        if (planExpired) {
    ...
    private fun openSettingsTo(section: String) {
        val i = Intent(this, com.nomitype.ui.MainActivity::class.java)
            .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
